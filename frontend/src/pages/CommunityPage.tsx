...
import { BarChart3, Bell, Eye, MessageCircle, Pencil, Search, Trash2, Users, X } from "lucide-react";
...
export default function CommunityPage() {
  ...
  useEffect(() => { if (tab !== "guide") void loadPosts(); }, [category, query, tab, page]);
  ...
  return <main className="mx-auto max-w-[1180px] px-6 py-8">
    <header className="border-b border-stone-300 pb-0"><div className="flex flex-col justify-between gap-5 pb-6 sm:flex-row sm:items-end"><div className="flex items-start gap-4"><span className="flex h-11 w-11 items-center justify-center rounded-lg bg-emerald-50 text-brand"><Users size={21}/></span><div><p className="text-[10px] font-bold text-brand">충북올겨 커뮤니티</p><h1 className="mt-1 text-[26px] font-bold">충북 생활 이야기</h1><p className="mt-2 text-xs text-stone-500">정착 경험과 지역 정보를 이웃과 나눠보세요.</p></div></div><div className="flex items-center gap-5 text-[11px] text-stone-500"><span><b className="mr-1 text-stone-800">등록된 글</b>{posts.length}</span></div></div><nav className="flex gap-6 text-xs font-semibold">{[["home","커뮤니티 홈"],["popular","인기글"],["guide","이용 안내"]].map(([id,label])=><button key={id} onClick={()=>{setTab(id as typeof tab);setPage(1);}} className={`border-b-2 pb-3 ${tab===id?"border-brand text-brand":"border-transparent text-stone-500"}`}>{label}</button>)}</nav></header>
    {error && <div className="mt-4 rounded-lg bg-red-50 px-4 py-3 text-xs text-red-700">{error}</div>}
    {tab==="guide"?<section className="mx-auto mt-8 max-w-4xl border-y border-stone-300 bg-white py-8"><div className="px-7"><p className="text-[10px] font-bold text-brand">COMMUNITY GUIDE</p><h2 className="mt-2 text-2xl font-bold">커뮤니티 이용수칙</h2><p className="mt-3 text-xs leading-6 text-stone-500">충북 생활 경험을 안전하게 나누기 위한 기준입니다. 게시글과 댓글 모두 동일하게 적용됩니다.</p></div><div className="mt-7 grid border-t border-stone-200 md:grid-cols-2">{[["01","개인정보와 사생활 보호","전화번호·상세 주소·직장·학교 등 개인을 특정할 수 있는 정보는 게시하지 마세요."],["02","존중하는 대화","욕설, 조롱, 혐오 표현과 지역·성별·직업 등을 이유로 한 비하를 금지합니다."],["03","정확한 지역·정책 정보","정책, 비용, 교통 정보는 가능한 경우 출처와 확인 날짜를 함께 적어주세요."],["04","광고와 거래 제한","상업 광고, 반복 홍보, 금전 거래 유도 및 불법 중개 게시물은 제한합니다."],["05","저작권과 초상권","직접 작성했거나 사용 허가를 받은 글과 이미지만 게시해 주세요."],["06","신고와 운영 조치","운영규칙을 위반한 게시물은 검토 후 숨김·삭제되거나 작성이 제한될 수 있습니다."]].map(([number,title,description],index)=><article key={number} className={`px-7 py-6 ${index%2===0?"md:border-r":""} ${index<4?"border-b border-stone-200":""}`}><div className="flex gap-4"><b className="text-sm text-brand">{number}</b><div><h3 className="text-sm font-bold">{title}</h3><p className="mt-2 text-xs leading-6 text-stone-500">{description}</p></div></div></article>)}</div></section>:<div className="mt-5 grid gap-5 lg:grid-cols-[190px_1fr_250px]">
      ...
      <section className="overflow-hidden rounded-xl border border-stone-200 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-200 p-5"><div><h2 className="text-lg font-bold">{tab==="popular"?"인기글":category}</h2><p className="mt-1 text-[10px] text-stone-400">게시글 {posts.length}개</p></div><div className="relative"><input value={query} onChange={(event)=>{setQuery(event.target.value);setPage(1);}} className="h-9 rounded-lg border border-stone-300 pl-3 pr-9 text-xs" placeholder="제목 또는 작성자 검색"/><Search size={14} className="absolute right-3 top-3 text-stone-400"/></div></div>
