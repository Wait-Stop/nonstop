...
import { ArrowLeft, BusFront, Car, CheckCircle2, Clock3, Heart, Hospital, Images, Info, MapPin, Save, Store, X } from "lucide-react";
...
export function RegionDetailPage() {
  ...
  const [evidenceOpen, setEvidenceOpen] = useState<"commute"|"car"|"policies"|null>(null);
  useEffect(() => {
    let active = true;
    setRelatedPolicies([]);
    api.getRegion(id).then(async (item) => {
      ...
      setRegion(item);
      const policies = await Promise.all((item.relatedPolicyIds || []).map((policyId) => api.getPolicy(policyId)));
      if (active) setRelatedPolicies(policies.filter((policy): policy is Policy => Boolean(policy)));
    });
    return () => { active = false; };
  }, [id]);
  ...
  const carNeedLevel = transportScore===undefined?(region.carNeed==="필요"?"상":region.carNeed==="선택"?"하":"중"):transportScore>=70?"하":transportScore>=50?"중":"상";
  ...
  return <main className="mx-auto max-w-[1100px] px-6 py-9"><Link to="/recommendations" className="flex items-center gap-1 text-xs text-stone-500"><ArrowLeft size={14}/>추천 결과로</Link><div className="relative mt-5 overflow-hidden rounded-2xl bg-brand p-8 text-white"><img src={region.image} alt={`${region.area} 풍경`} className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/55 to-black/20"/><div className="relative"><p className="text-sm text-white/70">{region.type}</p><div className="mt-2 flex items-end justify-between"><div><h1 className="text-3xl font-bold">{region.name}</h1><p className="mt-3 text-sm text-white/75">{region.reasons.join(" · ")}</p></div><div className="text-right"><strong className="text-5xl">{region.score}</strong><span className="text-sm"> / 100</span><p className="text-xs text-white/70">나와의 적합도</p></div></div></div></div>
    ...
    <section className="mt-5 grid gap-4 md:grid-cols-3">{[["commute",Clock3,`평균 출퇴근 : ${region.commute}분`],["car",Car,`차량 필요도 : ${carNeedLevel}`],["policies",CheckCircle2,`받을 수 있는 정책 : ${policyCount}개`]].map(([key,Icon,value])=><button key={String(key)} onClick={()=>setEvidenceOpen(key as typeof evidenceOpen)} className="rounded-xl border border-stone-200 bg-white p-5 text-left transition hover:border-brand hover:shadow-card"><Icon className="text-brand"/><strong className="mt-4 block text-xl">{String(value)}</strong><span className="mt-1 flex items-center gap-1 text-xs text-stone-400"><Info size={12}/>클릭해 산정 근거 보기</span></button>)}</section>
    ...
    {evidenceOpen&&<div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/45 px-5" onMouseDown={(event)=>{if(event.target===event.currentTarget)setEvidenceOpen(null);}}><section role="dialog" aria-modal="true" className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-7 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold text-brand">산정 근거</p><h2 className="mt-2 text-xl font-bold">{evidenceOpen==="commute"?`평균 출퇴근 : ${region.commute}분`:evidenceOpen==="car"?`차량 필요도 : ${carNeedLevel}`:`받을 수 있는 정책 : ${policyCount}개`}</h2></div><button onClick={()=>setEvidenceOpen(null)} aria-label="닫기" className="rounded-lg p-1 text-stone-400 hover:bg-stone-100"><X size={20}/></button></div>{evidenceOpen==="commute"&&<div className="mt-6 space-y-4 text-sm"><div className="rounded-xl bg-stone-50 p-4"><span className="text-xs text-stone-400">대표 이동 구간</span><b className="mt-1 block">{region.commuteBasis?.origin||region.name} → {region.commuteBasis?.destination||region.infrastructure[0]}</b></div><p className="leading-6 text-stone-600">{region.commuteBasis?.method||"백엔드 지역별 대표 생활권 기준 편도 예상시간"}으로 설정된 값입니다.</p><p className="rounded-lg bg-amber-50 p-4 text-xs leading-5 text-amber-800">{region.commuteBasis?.caution||"실제 소요 시간은 출발지, 근무지, 시간대와 교통수단에 따라 달라질 수 있습니다."}</p></div>}{evidenceOpen==="car"&&<div className="mt-6 space-y-4 text-sm"><div className="rounded-xl bg-stone-50 p-4"><span className="text-xs text-stone-400">AI 교통 접근성 점수</span><b className="mt-1 block text-2xl text-brand">{transportScore===undefined?"확인 불가":`${transportScore}점 / 100점`}</b></div><div className="grid grid-cols-3 gap-2 text-center text-xs"><span className={`rounded-lg p-3 ${carNeedLevel==="하"?"bg-brand text-white":"bg-stone-50"}`}>70점 이상<br/><b>하</b></span><span className={`rounded-lg p-3 ${carNeedLevel==="중"?"bg-brand text-white":"bg-stone-50"}`}>50~69점<br/><b>중</b></span><span className={`rounded-lg p-3 ${carNeedLevel==="상"?"bg-brand text-white":"bg-stone-50"}`}>50점 미만<br/><b>상</b></span></div><p className="leading-6 text-stone-600">AI 지역 데이터의 대중교통 접근성 점수를 기준으로 차량 의존도를 역산했습니다. 교통 접근성이 높을수록 차량 필요도는 낮아집니다.</p></div>}{evidenceOpen==="policies"&&<div className="mt-6"><p className="text-sm leading-6 text-stone-600">백엔드가 이 지역에 연결한 정책 ID를 정책 API에서 다시 조회한 결과입니다. 아래 정책은 신청 가능성을 확정하지 않으며 세부 자격 확인이 필요합니다.</p><div className="mt-4 space-y-3">{relatedPolicies.map((policy)=><Link key={policy.id} to={`/policies/${policy.id}`} onClick={()=>setEvidenceOpen(null)} className="block rounded-xl border border-stone-200 p-4 hover:border-brand"><span className="text-[10px] font-bold text-brand">{policy.category} · {policy.status||"확인 필요"}</span><b className="mt-1 block text-sm">{policy.title}</b><p className="mt-2 text-xs text-stone-500">{policy.benefit}</p></Link>)}{!relatedPolicies.length&&<p className="rounded-lg bg-stone-50 p-4 text-xs text-stone-500">정책 상세 정보를 불러오지 못했습니다.</p>}</div></div>}</section></div>}
  </main>;
}
