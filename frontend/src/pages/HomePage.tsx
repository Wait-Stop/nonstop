...
export default function HomePage() {
  ...
  const submitQuick = () => {
    ...
    const condition: QuickCondition = {
      ...
      recommendRegion: recommend,
    };
    localStorage.setItem(
      "chungbuk-olgyeo-quick-condition",
    ...
    <main>
      <section className="mx-auto grid max-w-[1240px] gap-8 px-6 py-10 lg:grid-cols-[1fr_1.05fr]">
        <div className="flex min-h-[250px] flex-col justify-center">
          <span className="mb-4 w-fit rounded-full bg-brand-light px-3 py-1.5 text-[11px] font-bold text-brand">
            충북 정착 생활 플랫폼
          </span>
          <h1 className="text-[32px] font-bold leading-[1.4] tracking-[-0.05em]">
            충북에서의 새로운 시작,
            <br />
            <span className="text-brand">충북올겨</span>와 함께 준비해보세요
          </h1>
          <p className="mt-4 text-sm leading-6 text-stone-500">
            내게 맞는 지역을 찾고, 실제 생활비와 출퇴근을 미리 경험하고,
            <br className="hidden md:block" />
            정착에 필요한 정책까지 한눈에 확인하세요.
          </p>
        </div>
        <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-card">
          <h2 className="text-[15px] font-bold">어떤 시뮬레이션을 해볼까요?</h2>
          <p className="mt-1 text-[11px] text-stone-400">
            원하는 항목을 선택한 뒤 시작 버튼을 눌러주세요.
          </p>
            ...
            {SIMULATIONS.map(({ id, icon: Icon, title, sub }) => {
              ...
              return (
                <button
                  ...
                  className={`flex h-[126px] flex-col items-center justify-center rounded-xl border bg-white text-center transition-all ${selected ? "border-brand ring-2 ring-brand/10" : "border-stone-200 hover:border-brand"}`}
                >
                  <Icon size={29} strokeWidth={1.6} className="text-brand" />
                  <strong className="mt-3 text-[12px] text-stone-800">
                    {title}
                  </strong>
                  <span className="mt-1 text-[9px] text-stone-400">{sub}</span>
                </button>
              );
            })}
          </div>
          <div className="mt-4 flex justify-end">
            <button
              ...
              className="rounded-lg bg-brand px-7 py-2.5 text-xs font-bold text-white hover:bg-brand-dark disabled:cursor-not-allowed disabled:bg-stone-300"
            >
              선택한 시뮬레이션 시작하기
            </button>
          </div>
        ...
        <div className="rounded-2xl border border-emerald-100 bg-[#F3FAF6] p-5">
          <div className="mb-4 flex items-end justify-between">
            <div>
              <h2 className="text-lg font-bold">
                내 정착 조건으로 맞는 지역을 찾아보세요
              </h2>
              <p className="mt-1 text-xs font-medium text-brand">
                로그인 없이 간단하게 추천받을 수 있으며, 이 결과는 계정에
                저장되지 않습니다.
              </p>
            </div>
            <span className="rounded-full bg-white px-3 py-1 text-[10px] font-bold text-brand">
              비회원 이용 가능
            </span>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
            {FIELDS.map(([key, label, options]) => (
              <label key={key}>
                <span className="mb-1.5 block text-[11px] font-medium text-stone-500">
                  {label}
                </span>
                <span className="relative block">
                  <select
                    ...
                    className="h-10 w-full appearance-none rounded-lg border border-stone-300 bg-white px-3 pr-8 text-xs outline-none focus:border-brand"
                  >
            ...
            <div className="relative">
              ...
              <button
                ...
                className="flex h-10 w-full items-center justify-between rounded-lg border border-stone-300 bg-white px-3 text-xs"
              >
              ...
              {regionOpen && (
                <div className="absolute right-0 top-16 z-20 w-64 rounded-xl border border-stone-200 bg-white p-3 shadow-xl">
                  <button
                    onClick={chooseRecommend}
                    className={`mb-2 w-full rounded-lg border px-3 py-2 text-left text-xs font-bold ${recommend ? "border-brand bg-brand-light text-brand" : "border-stone-200"}`}
                  >
                    추천받을게요{" "}
                  ...
                  <div className="grid grid-cols-2 gap-1 border-t border-stone-100 pt-2">
                    {REGIONS.map((region) => (
                      <label
                        key={region}
                        className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-xs hover:bg-stone-50"
                      >
                        <input
                          type="checkbox"
                  ...
                  <button
                    ...
                    className="mt-2 w-full rounded-lg bg-brand py-2 text-xs font-bold text-white"
                  >
                    선택 완료
                  </button>
                </div>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <button
              onClick={submitQuick}
              className="flex items-center gap-1.5 rounded-lg bg-brand px-6 py-2.5 text-xs font-bold text-white"
            >
              <Search size={14} />
              지역 찾기
            </button>
        ...
        <div className="mx-auto max-w-[1240px] px-6 py-16">
          <div className="grid gap-8 border-b border-emerald-200/70 pb-10 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
            ...
            <div className="flex flex-col items-start justify-between gap-5 sm:flex-row sm:items-end">
              <p className="max-w-xl text-[13px] leading-6 text-stone-600">
                막연한 지역 소개 대신 내 조건에 맞는 후보지를 찾고, 주거비와
                이동 시간을 비교한 뒤 받을 수 있는 지원정책까지 순서대로 확인할
                수 있습니다.
              </p>
              <Link
                to="/regions"
                className="flex shrink-0 items-center gap-2 border-b border-brand pb-1 text-xs font-bold text-brand"
              >
          ...
          <div className="grid border-b border-emerald-200/70 py-9 md:grid-cols-3">
            {[
              ...
              <div
                key={String(number)}
                className={`relative flex gap-4 py-3 ${index ? "md:border-l md:border-emerald-200/70 md:pl-8" : ""} ${index < 2 ? "md:pr-8" : ""}`}
              >
                ...
                <div>
                  <span className="text-[10px] font-bold tracking-[.12em] text-brand">
                    {String(number)} STEP
                  </span>
          ...
          <div className="mt-8 overflow-hidden border-y border-stone-300 bg-white">
            <div className="grid lg:grid-cols-[340px_1fr]">
              <div className="border-b border-stone-200 px-7 py-8 lg:border-b-0 lg:border-r">
                ...
                <p className="mt-3 text-xs leading-5 text-stone-500">
                  지역 추천 결과는 로그인 없이 볼 수 있고 계정에는 저장되지
                  않습니다.
                </p>
                <Link
              ...
              <div className="divide-y divide-stone-200">
                {[
                  ...
                  >
                    ...
                    <span>
                      <small className="block text-[10px] font-bold text-brand">
                        {String(category)}
                      </small>
            ...
            <div className="flex flex-wrap gap-x-8 gap-y-2 border-t border-stone-200 bg-[#FAFAF8] px-7 py-3 text-[10px] text-stone-500">
              <span>
                <strong className="mr-1.5 text-stone-700">지역 정보</strong>추천
                대상 5개 지역
              </span>
              <span>
                <strong className="mr-1.5 text-stone-700">생활 도구</strong>
                생활비·출퇴근·하루 살기·지출
              </span>
              <span>
                <strong className="mr-1.5 text-stone-700">지원 분야</strong>
                주거·취업·창업·귀농귀촌
              </span>
            </div>
          </div>
