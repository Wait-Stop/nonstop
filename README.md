...
- Gemini API 기반 충북 정착 상담
- 백엔드 지역·정책 데이터를 활용한 검색 증강 생성(RAG)

프론트엔드는 NestJS 백엔드 API를 호출하고, 백엔드는 별도로 실행되는 FastAPI 추천 서비스와 연동합니다. AI 서비스가 응답하지 않으면 백엔드의 규칙 기반 추천 로직으로 자동 전환합니다.

### Gemini 정착 상담과 RAG

Gemini 상담은 모델의 가중치를 별도로 학습하는 파인튜닝 방식이 아니라, 서비스가 보유한 최신 데이터를 질문마다 검색해 함께 전달하는 검색 증강 생성(RAG) 방식으로 구현했습니다.

```text
사용자 질문
  → 교통·주거·정책·일자리·생활비 주제 분류
  → 질문과 선택 지역에서 검색어 추출
  → 백엔드의 5개 지역 및 정책 데이터 검색
  → 관련 지역 최대 2개, 관련 정책 최대 4개 선별
  → 사용자 조건·시뮬레이션 결과와 함께 Gemini에 전달
  → 검색 자료에 근거한 답변 생성
```

- 지역 검색 자료: 평균 임대료, 평균 출퇴근 시간, 교통 점수, 차량 필요도, 생활 인프라, 직무 특성
- 정책 검색 자료: 대상 지역, 지원 내용, 자격 조건, 담당 기관, 출처, 확인 기준일
- 정책 자료는 사용자가 정책·지원·혜택을 질문한 경우에만 전달합니다.
- 검색 자료에 없는 자전거 도로, 경사도, 배차 간격 또는 정책 조건은 Gemini가 임의로 생성하지 않도록 제한합니다.
- 답변은 질문한 주제에만 집중하며, 정책 관련 내용은 공식 출처와 최신 공고를 다시 확인하도록 안내합니다.

따라서 지역·정책 데이터가 변경되면 모델을 다시 학습하지 않고 백엔드 데이터만 갱신해 상담 답변에 반영할 수 있습니다.
